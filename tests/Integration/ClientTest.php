...
class ClientTest extends ClientIntegrationTest
{
    ...
    public function testGettersSetters()
    {
        ...
        $this->assertEquals('frankmayer\ArangoDbPhpCoreCurl\Protocols\Http\HttpRequest', $getRequestOriginalClass);

        $object = $this->client->setRequestClass('frankmayer\ArangoDbPhpCoreCurl\Protocols\Http\HttpRequestTest');
        $this->assertTrue($this->client === $object);

        $getRequestClass = $this->client->getRequestClass();
        $this->assertEquals('frankmayer\ArangoDbPhpCoreCurl\Protocols\Http\HttpRequestTest', $getRequestClass);

        $object = $this->client->setRequestClass($getRequestOriginalClass);
        $this->assertTrue($this->client === $object);

        $getRequestClass = $this->client->getRequestClass();
        $this->assertEquals('frankmayer\ArangoDbPhpCoreCurl\Protocols\Http\HttpRequest', $getRequestClass);

        $getResponseOriginalClass = $this->client->getResponseClass();
        $this->assertEquals('frankmayer\ArangoDbPhpCoreCurl\Protocols\Http\HttpResponse', $getResponseOriginalClass);

        $object = $this->client->setResponseClass('frankmayer\ArangoDbPhpCoreCurl\Protocols\Http\HttpResponseTest');
        $this->assertTrue($this->client === $object);

        $getResponseClass = $this->client->getResponseClass();
        $this->assertEquals('frankmayer\ArangoDbPhpCoreCurl\Protocols\Http\HttpResponseTest', $getResponseClass);

        $object = $this->client->setResponseClass($getResponseOriginalClass);
        $this->assertTrue($this->client === $object);

        $getResponseClass = $this->client->getResponseClass();
        $this->assertEquals('frankmayer\ArangoDbPhpCoreCurl\Protocols\Http\HttpResponse', $getResponseClass);
    }
